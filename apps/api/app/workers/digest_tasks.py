...
def process_hourly_digests_task() -> int:
    ...
    async def _run():
        ...
        async with async_session_factory() as session:
            stmt = select(DigestSubscription).where(DigestSubscription.enabled == True)
            ...
            for freq, user_ids in triggered_frequencies_by_user.items():
                logger.info("Triggering digest compilation and delivery for frequency '%s' for %d users.", freq, len(user_ids))
...
def trigger_digest_delivery_now_task(frequency: str) -> int:
    ...
    async def _run():
        async with async_session_factory() as session:
            delivered = await digest_delivery_service.compile_and_deliver_digests(session, frequency)
            return delivered
