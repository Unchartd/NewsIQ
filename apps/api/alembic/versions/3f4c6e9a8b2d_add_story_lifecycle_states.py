...
def upgrade() -> None:
    ...
    op.add_column(
        "stories",
        sa.Column(
            "lifecycle_state", sa.String(length=30), nullable=False, server_default="emerging"
        ),
    )
    ...
    op.add_column(
        "stories", sa.Column("last_discovery_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "stories",
        sa.Column(
    ...
    op.create_index(
        op.f("ix_stories_lifecycle_state"), "stories", ["lifecycle_state"], unique=False
    )
    op.create_index(
        op.f("ix_stories_canonical_event_id"), "stories", ["canonical_event_id"], unique=False
    )
    op.create_index(
        op.f("ix_stories_lifecycle_changed_at"), "stories", ["lifecycle_changed_at"], unique=False
    )
    op.create_index(
        op.f("ix_stories_last_discovery_at"), "stories", ["last_discovery_at"], unique=False
    )
    op.create_index(
        op.f("ix_stories_confidence_score"), "stories", ["confidence_score"], unique=False
    )
    op.create_index(
        op.f("ix_stories_freshness_score"), "stories", ["freshness_score"], unique=False
    )
    op.create_index(
        op.f("ix_stories_source_diversity_count"),
        "stories",
        ["source_diversity_count"],
        unique=False,
    )
    op.create_index(
        op.f("ix_stories_contradiction_score"), "stories", ["contradiction_score"], unique=False
    )
