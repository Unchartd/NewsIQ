...
class GatewayRequest(BaseModel):
    ...
    model: str = Field(
        ..., description="Target model ID (e.g., gemini-2.5-flash-lite, gpt-4o-mini)"
    )
    messages: list[dict[str, Any]] = Field(
        ...,
        description="OpenAI-style message list: [{'role': 'user/system/assistant', 'content': '...'}]",
    )
    temperature: float = Field(0.0, description="Sampling temperature")
    response_format: dict[str, Any] | type[BaseModel] | None = Field(
        None, description="Pydantic class or JSON schema for structured outputs"
    )
    tools: list[dict[str, Any]] | None = Field(
        None, description="List of available tools/functions"
    )
    tool_choice: str | dict[str, Any] | None = Field(None, description="Tool choice constraint")
    stage: str = Field(
        "unknown", description="Pipeline stage (e.g. cluster_verification, summary_reflection)"
    )
...
class GatewayResponse(BaseModel):
    ...
    provider: str = Field(
        ..., description="The model provider that fulfilled the call (e.g., google, openai, groq)"
    )
    model: str = Field(..., description="The specific model used")
    key_used: str | None = Field(None, description="Masked/hashed version of the API key used")
