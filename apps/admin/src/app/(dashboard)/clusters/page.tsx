        ...
        <div className="glass rounded-2xl p-5 space-y-3">
          ...
          <div className="space-y-2">
            <input value={mergeSource} onChange={(e) => setMergeSource(e.target.value)} placeholder="Source Cluster ID"
              className="w-full px-3 py-2 rounded-xl bg-[#1a1f2e] border border-[#1e2333] text-slate-200 text-xs placeholder-slate-600 focus:outline-none focus:border-indigo-500/60 transition-all font-mono" />
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {isLoading ? (
              ...
              clusters.map((c) => (
                <tr key={c.id} className="border-b border-[#1e2333]/50 hover:bg-white/2 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Link href={`/dashboard/stories/${c.id}`} className="text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors">
