...
async def _check_discovery_task_completion(discovery_task_id: uuid.UUID, session: Any) -> None:
    from sqlalchemy import select
    from datetime import UTC, datetime
    from app.models.models import DiscoveryTask, DiscoveryTaskState, CrawlTask, CrawlTaskState
    ...
    if all_done:
        ...
        if parent_task and parent_task.status != DiscoveryTaskState.COMPLETE:
            ...
            logger.info("DiscoveryTask %s marked COMPLETE (all crawl tasks completed)", discovery_task_id)


@celery_app.task(name="app.workers.tasks.discovery_search_task")
def discovery_search_task(discovery_task_id_str: str, run_id: str | None = None, trace_id: str | None = None) -> None:
    ...
    async def _run():
        ...
        async with async_session_factory() as session:
            ...
            if task.status != DiscoveryTaskState.PENDING:
                logger.info("DiscoveryTask %s is not PENDING (current status: %s). Skipping.", discovery_task_id_str, task.status)
                return
            ...
            try:
                ...
                if current_searches >= settings.DISCOVERY_DAILY_SEARCH_BUDGET:
                    logger.warning("Daily discovery search budget exceeded (%d/%d). Expiring task %s.", 
                                   current_searches, settings.DISCOVERY_DAILY_SEARCH_BUDGET, discovery_task_id_str)
            ...
            if not original_article:
                logger.error("Original article %s not found for DiscoveryTask %s.", task.article_id, discovery_task_id_str)
            ...
            try:
                ...
                if not acquired:
                    logger.info("Duplicate search lock hit for query '%s' on provider %s. Expiring task %s.", 
                                task.query, task.provider, discovery_task_id_str)
                    task.status = DiscoveryTaskState.EXPIRED
            ...
            task.queued_at = datetime.now(UTC).replace(tzinfo=None) if not task.queued_at else task.queued_at
            await session.flush()
            ...
            try:
                ...
                raw_results = await asyncio.wait_for(
                    provider.search(query=task.query, max_results=settings.DISCOVERY_MAX_RESULTS * 3),
                    timeout=5.0
                )
                ...
                logger.error("Discovery search provider failed for task %s: %s", discovery_task_id_str, e)
                task.retry_count += 1
                if task.retry_count < settings.DISCOVERY_MAX_RETRIES:
                    task.status = DiscoveryTaskState.PENDING
                    task.next_retry_at = datetime.now(UTC).replace(tzinfo=None) + timedelta(minutes=2 ** task.retry_count)
            ...
            if not discovered_urls:
                logger.info("No URLs discovered for query '%s'. Completing task %s.", task.query, discovery_task_id_str)
            ...
            for url in discovered_urls:
                url_canonical = gnews_service.canonicalize_url(url) if hasattr(gnews_service, "canonicalize_url") else url
...
def discovery_crawl_task(crawl_task_id_str: str, run_id: str | None = None, trace_id: str | None = None) -> None:
    ...
    async def _run():
        ...
        from sqlalchemy import select
        from app.core.config import settings
        from app.models.models import CrawlTask, CrawlTaskState, DiscoveryTask, Article, Source
        from app.services.cache_service import cache_service
        ...
        async with async_session_factory() as session:
            ...
            if crawl_task.status not in (CrawlTaskState.PENDING, CrawlTaskState.RETRYING):
                logger.info("CrawlTask %s is not in processable state (current status: %s). Skipping.", 
                            crawl_task_id_str, crawl_task.status)
                return
            ...
            try:
                ...
                if current_downloads >= settings.DISCOVERY_DAILY_DOWNLOAD_BUDGET:
                    logger.warning("Daily download budget exceeded (%d/%d). Failing CrawlTask %s.", 
                                   current_downloads, settings.DISCOVERY_DAILY_DOWNLOAD_BUDGET, crawl_task_id_str)
            ...
            if await url_bloom_filter.exists(crawl_task.url_hash):
                logger.info("Bloom Filter skip hit for CrawlTask %s (URL: %s)", crawl_task_id_str, crawl_task.url)
            ...
            try:
                ...
                logger.warning("Failed to crawl URL %s for CrawlTask %s: %s", crawl_task.url, crawl_task_id_str, e)
                ...
                if crawl_task.retry_count < settings.DISCOVERY_MAX_RETRIES:
                    ...
                    crawl_task.next_retry_at = datetime.now(UTC).replace(tzinfo=None) + timedelta(minutes=2 ** crawl_task.retry_count)
            ...
            if dup_content_res.scalar_one_or_none():
                logger.info("Content hash duplicate found in DB for CrawlTask %s", crawl_task_id_str)
                await url_bloom_filter.add(crawl_task.url_hash)
                crawl_task.status = CrawlTaskState.SUCCESS
            ...
            source = await gnews_service._resolve_source(
                gnews_source_name=None,
                gnews_source_url=crawl_task.url,
                session=session
            )
            if not source:
            ...
            try:
                ...
                logger.error("Failed to run downstream match coordinator for article %s: %s", new_article.id, e)
    ...
    async def _run():
        ...
        async with async_session_factory() as session:
            ...
            crawl_stmt = select(CrawlTask).where(
                CrawlTask.status == CrawlTaskState.RETRYING,
                CrawlTask.next_retry_at <= now
            )
            ...
            if counts["search_retries"] > 0 or counts["crawl_retries"] > 0:
                await session.commit()
                logger.info("Re-enqueued %d search discovery tasks and %d crawl tasks for retry.", 
                            counts["search_retries"], counts["crawl_retries"])
            return counts
...
def cleanup_discovery_tasks_task() -> dict[str, int]:
    ...
    async def _run():
        ...
        async with async_session_factory() as session:
            ...
            if total_deleted > 0:
                await session.commit()
                logger.info("Discovery GC deleted %d tasks (Complete: %d, Failed: %d, Expired: %d).", 
                            total_deleted, count_complete, count_failed, count_expired)
