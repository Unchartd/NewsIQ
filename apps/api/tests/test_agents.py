...
async def test_cluster_verification_agent(mock_run):
    schema_instance = ClusterVerificationSchema(
        same_event=True, confidence=0.95, explanation="Both describe the same event."
    )

    mock_run_output = RunOutput(content=schema_instance, metrics=None)
...
async def test_entity_disambiguation_agent(mock_run):
    ...
    res = await disambiguate_entity(
        entity_value="Trump", entity_type="PERSON", context="President Trump gave a speech."
    )
...
async def test_contradiction_agent(mock_run):
    ...
    res = await check_contradiction(
        fact_type="event_time", val1="2 PM", val2="3 PM", source1_name="BBC", source2_name="TOI"
    )
