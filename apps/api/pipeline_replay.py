...
async def replay_pipeline(
    limit: int = 100, live: bool = False, dataset: str = "historical", compare: bool = False
):
    """
    ...
    async with async_session_factory() as session:
        ...
        if dataset == "yesterday":
            ...
            stmt = (
                select(Article)
                .where(Article.published_at >= one_day_ago)
            ...
            stmt = (
                select(Article)
                .where(Article.content is not None)
                .order_by(Article.published_at.desc())
                .limit(limit)
            )
        else:  # historical
        ...
        await session.execute(
            delete(ArticleEntity).where(ArticleEntity.article_id.in_(article_ids))
        )
        await session.execute(
            delete(DiscoveryQueue).where(DiscoveryQueue.article_id.in_(article_ids))
        )
        ...
        await session.execute(
            ...
            .values(embedding_status="pending", event_extraction_status="pending")
        )
        ...
        for art in articles:
            ...
            # Step 1: Embed
            text_to_embed = (
                f"{art.title or ''} {art.description or ''} {(art.content or '')[:4000]}".strip()
            )
            ...
            try:
                ...
                if vectors:
                    ...
                    await vector_service.upsert_article(
                        article_id=art.id, vector=vectors[0], payload=payload
                    )
            ...
            try:
                event_response = await event_service.extract_events(
                    title=art.title or "", content=content, published_at=pub_at
                )
        ...
        stmt_sa = select(StoryArticle.article_id, StoryArticle.story_id).where(
            StoryArticle.article_id.in_(article_ids)
        )
        ...
        avg_cluster_size = (
            sum(len(v) for v in clusters.values()) / cluster_count if cluster_count > 0 else 0.0
        )
        ...
        try:
            ...
            if len(clustered_aids) >= 2 and cluster_count >= 2:
                points = await vector_service.client.retrieve(
                    ...
                    ids=[str(aid) for aid in clustered_aids],
                    with_vectors=True,
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="historical",
        choices=["historical", "yesterday", "custom"],
        help="Source dataset",
    )
    parser.add_argument(
        "--compare", action="store_true", help="Compare with the previous replay runs"
    )
