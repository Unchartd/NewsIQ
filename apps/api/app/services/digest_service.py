...
from app.models.models import DigestSubscription, UserPreference, UserCategory, Category, Story, Notification
...
class DigestDeliveryService:
    ...
    async def compile_and_deliver_digests(self, db: AsyncSession, frequency: str) -> int:
        ...
        stmt = (
            ...
            .where(
                DigestSubscription.frequency == frequency,
                DigestSubscription.enabled == True
            )
        ...
        for user_id, udata in user_subscriptions.items():
            ...
            try:
                ...
                cat_query = select(Category.id).join(UserCategory).where(UserCategory.user_id == user_id)
                ...
                if not stories:
                    fallback_query = select(Story).order_by(Story.created_at.desc()).limit(story_count)
                    f_result = await db.execute(fallback_query)
                    stories = f_result.scalars().all()
                    
                if not stories:
                    logger.warning("Digest delivery: No stories available to compile digest for user %s.", user.email)
                    continue
                ...
                for channel in channels:
                    if channel == "email":
                        await self.email_service.send_digest_email(user, edition_title, stories_payload)
                        logger.info("Digest delivery: Sent email digest to %s for edition '%s'.", user.email, edition_title)
                    elif channel == "in_app":
                        ...
                        db.add(notification)
                        logger.info("Digest delivery: Created in-app notification for %s for edition '%s'.", user.email, edition_title)
                
                processed_count += 1
            except Exception as e:
                logger.error("Digest delivery: Failed to compile or deliver digest for user %s: %s", user.email, e)
