...
class AgentRegistry:
    ...
    def get_agent(
        ...
        if name == "cluster_verification":
            ...
            agent.model = GatewayModel(id=model_id, stage="cluster_verification", story_id=story_id, article_id=article_id)
            return agent
            ...
            agent.model = GatewayModel(id=model_id, stage="entity_disambiguation", story_id=story_id, article_id=article_id)
            return agent

        elif name == "contradiction":
            from app.agents.contradiction_agent import contradiction_agent
            agent = contradiction_agent
            agent.model = GatewayModel(id=model_id, stage="contradiction_detection", story_id=story_id, article_id=article_id)
            return agent

        elif name == "reflection":
            from app.agents.reflection_agent import reflection_agent
            agent = reflection_agent
            agent.model = GatewayModel(id=model_id, stage="summary_reflection", story_id=story_id, article_id=article_id)
            return agent

        elif name == "judge":
            from app.agents.judge_agent import judge_agent
            agent = judge_agent
            agent.model = GatewayModel(id=model_id, stage="judge_arbitration", story_id=story_id, article_id=article_id)
            return agent
