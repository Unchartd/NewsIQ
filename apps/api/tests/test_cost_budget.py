...
async def test_stage_aware_budget_enforcement():
    ...
    with patch.object(cache_service, "_redis", None):
        ...
        assert not await cost_budget_manager.is_stage_budget_exceeded(story_id, "summary_reflection", category)
        assert not await cost_budget_manager.is_stage_budget_exceeded(story_id, "source_comparison", category)

        # 2. Exceeds reflection budget (50% of $0.015 = $0.0075)
        await cost_budget_manager.add_story_cost(story_id, 0.006) # $0.008 total
        assert await cost_budget_manager.is_stage_budget_exceeded(story_id, "summary_reflection", category)
        assert not await cost_budget_manager.is_stage_budget_exceeded(story_id, "source_comparison", category)

        # 3. Exceeds comparison budget (80% of $0.015 = $0.012)
        await cost_budget_manager.add_story_cost(story_id, 0.005) # $0.013 total
        assert await cost_budget_manager.is_stage_budget_exceeded(story_id, "summary_reflection", category)
        assert await cost_budget_manager.is_stage_budget_exceeded(story_id, "source_comparison", category)
        assert not await cost_budget_manager.is_stage_budget_exceeded(story_id, "summary_generation", category)
