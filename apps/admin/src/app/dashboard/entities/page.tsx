    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-background border border-border text-foreground text-sm placeholder-slate-650 focus:outline-none focus:border-primary/60 transition-all"
        />
      </div>
        ...
        <table className="w-full text-xs">
          ...
          <tbody>
            {isLoading ? (
              ...
              entities.map((entity) => (
                <tr key={entity.id} className="border-b border-border/50 hover:bg-white/2 transition-colors">
                    ...
                    {editingId === entity.id ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          placeholder="Q12345"
                          autoFocus
                          className="w-24 px-2 py-1 rounded-lg bg-background border border-primary/60 text-slate-200 text-xs font-mono focus:outline-none"
                        />
                        <button
                          onClick={() => overrideMutation.mutate({ entityId: entity.id, wikidataId: wikidataInput })}
                          className="text-emerald-400 hover:text-emerald-300"
                        ><Check className="w-3.5 h-3.5" /></button>
                        <button onClick={() => setEditingId(null)} className="text-slate-500 hover:text-slate-305">
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    ) : entity.wikidata_id ? (
                      <a href={`https://www.wikidata.org/wiki/${entity.wikidata_id}`} target="_blank" rel="noopener noreferrer"
                        className="text-primary hover:text-primary/80 flex items-center gap-1 font-mono">
                        {entity.wikidata_id} <ExternalLink className="w-3 h-3" />
                      </a>
                    ) : (
                      <span className="text-slate-655 italic">Not resolved</span>
                    )}
                  </td>
