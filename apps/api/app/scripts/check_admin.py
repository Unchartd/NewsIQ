...
async def check_and_create_admin():
    ...
    async with async_session_factory() as session:
        ...
        result = await session.execute(
            select(User).where(User.email == "admin@newsiq.io")
        )
        ...
        result = await session.execute(
            select(User).where(User.email == "admin@newsiq.com")
        )
        ...
        if admin_com:
            ...
            print("❇️ Updated/Promoted admin@newsiq.com to admin role with password 'adminpassword123'")
        else:
