...
def test_should_prioritize_discovery():
    ...
    valid_content = "This is a very long content string of more than three hundred characters to satisfy the minimum length filter requirement in the diagnostic pipeline test. " * 3
    ...
    ok, reason = ingestion_service.should_prioritize_discovery(valid_title, "Short content", pub_now)
    assert ok is False
    ...
    ok, reason = ingestion_service.should_prioritize_discovery(opinion_title, valid_content, pub_now)
    assert ok is False
...
def test_calculate_discovery_score_weighted():
    ...
    score, breakdown = ingestion_service.calculate_discovery_score(
        title="Opinion: why this is wrong",
        content=content,
        pub_date=pub_now
    )
    assert score < 0
