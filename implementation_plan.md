...
> [!IMPORTANT]
> **Monorepo Structure & Shared Branding Module**: Since there is no root workspace package manager setup (no root `package.json`), I propose creating a local `packages/branding` module. Both frontend applications (`apps/web` and `apps/admin`) will install this module using a relative `file:../../packages/branding` dependency. The Python backend (`apps/api`) will load these configurations via a generated JSON/YAML config or hardcode the backend-specific config in its own `core.config` to avoid complex cross-language sharing. Please approve this structure.
...
### [Frontend Shared Branding - packages/branding]
Create a centralized internal package for all frontend branding logic.
- **[NEW]** `packages/branding/package.json`
- **[NEW]** `packages/branding/index.ts` - Export constants (`BRAND_NAME`, `DOMAIN`, colors).
- **[NEW]** `packages/branding/colors.ts` - HSL color tokens for primary branding (e.g., `#EF4444` and `#C41E3A` consolidated into a unified red scale).
- **[NEW]** `packages/branding/logo.tsx` - Centralized React `<BrandLogo />` component.
- **[NEW]** `packages/branding/assets/` - Centralize all SVG assets here, moving them from `apps/web/public/brand/`.

### [Frontend Web - apps/web]
Migrate existing branding from local folders to the shared package.
- **[MODIFY]** `apps/web/package.json` - Add dependency `"@newsiq/branding": "file:../../packages/branding"`.
- **[MODIFY]** `apps/web/src/app/globals.css`, `landing.css`, `legal.css` - Replace hardcoded `#C41E3A` and `#EF4444` with CSS variables defined by the branding system.
- **[MODIFY]** `apps/web/src/app/landing-client.tsx`, `digest/setup/page.tsx` - Import colors from `@newsiq/branding`.
- **[MODIFY]** `apps/web/public/manifest.json` - Ensure `theme_color` is injected or strictly matches the centralized primary color.
- **[MODIFY]** `apps/web/src/app/icon.tsx`, `apple-icon.tsx`, `icon-[size]/route.tsx` - Use shared assets from `@newsiq/branding`.
- **[DELETE]** `apps/web/src/branding/` - Remove old isolated branding module.
- **[DELETE]** `apps/web/public/brand/` - Remove old assets.

### [Frontend Admin - apps/admin]
Migrate the admin app to use the centralized branding system.
- **[MODIFY]** `apps/admin/package.json` - Add dependency `"@newsiq/branding": "file:../../packages/branding"`.
- **[MODIFY]** `apps/admin/src/app/page.tsx` - Remove inline SVGs and hardcoded "NewsIQ"; use `<BrandLogo />` and `BRAND_NAME` from `@newsiq/branding`.
- **[MODIFY]** `apps/admin/src/app/admin/layout.tsx` - Replace inline SVG with `<BrandLogo />`.
- **[MODIFY]** `apps/admin/src/app/globals.css` - Replace hardcoded `#EF4444` with CSS variables.
...
- Run `npm run build` on both `apps/web` and `apps/admin` to verify successful resolution of the `@newsiq/branding` package.
