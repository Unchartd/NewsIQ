...
class EntityDisambiguationSchema(BaseModel):
    ...
    wikidata_id: str | None = Field(
        None, description="The Wikidata ID (QID) if found, otherwise null."
    )
...
async def disambiguate_entity(
    entity_value: str, entity_type: str, context: str = ""
    ...
    run_output = await run_agent_with_observability(
        agent=entity_disambiguation_agent, prompt=prompt, stage="entity_disambiguation"
    )
