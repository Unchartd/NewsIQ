...
from app.llm_gateway.base_provider import APIKey, BaseLLMProvider
from app.llm_gateway.health_monitor import HealthMonitor
from app.llm_gateway.provider_pool import APIKeyPool, GeminiProvider, MockProvider, OpenAIProvider
...
class ProviderRouter:
    ...
    def __init__(
        self, key_pool: APIKeyPool, rate_limiter: RateLimitManager, health_monitor: HealthMonitor
    ) -> None:
        ...
        self.clients: dict[str, BaseLLMProvider] = {
            ...
            "cerebras": OpenAIProvider(
                provider_name="cerebras", base_url="https://api.cerebras.ai/v1"
            ),
            "nvidia": OpenAIProvider(
                provider_name="nvidia", base_url="https://integrate.api.nvidia.com/v1"
            ),
            "mock": MockProvider(),
        }
    ...
    def select_key_and_client(self, provider: str, model: str) -> tuple[APIKey, BaseLLMProvider]:
        ...
        if not healthy_keys:
            logger.error(
                "All keys for provider %s are flagged UNHEALTHY. Attempting fallback on last key.",
                provider,
            )
        ...
        if not available_keys:
            logger.warning(
                "All keys for provider %s are in cooldown. Selecting key with earliest cooldown expiration.",
                provider,
            )
            available_keys = sorted(healthy_keys, key=lambda k: k.cooldown_until or datetime.min)

        # Filter: Rate limiter limits check
        rate_ok_keys = []
        for key in available_keys:
            if self.rate_limiter.check_rate_limit(
                key.key, key.requests_per_minute, key.requests_per_day
            ):
