...
class _FakeRedis:
    ...
    async def eval(self, script, numkeys, *args):
        # Only the compare-and-delete / compare-and-expire behind
        # cache_service.delete_if_equals and expire_if_equals. Any other
        # script fails loudly instead of silently doing nothing. TTLs are not
        # simulated, so an "expire" only reports whether the caller owns it.
        owns = numkeys == 1 and "redis.call('get', KEYS[1]) == ARGV[1]" in script
        if owns and "'del'" in script:
            key, value = args
            if self._store.get(key) == value:
                del self._store[key]
                return 1
            return 0
        if owns and "'expire'" in script:
            key, value, _ttl = args
            return 1 if self._store.get(key) == value else 0
