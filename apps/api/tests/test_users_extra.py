...
from app.models.models import User, Notification, DigestSubscription, Story
...
async def test_update_digest_subscriptions(mock_db_session):
    ...
    response = await update_digest_subscriptions(
        body=body, user=user, db=mock_db_session
    )
